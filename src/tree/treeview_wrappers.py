##  This module is from Mark Lutz's Programming Python, 3rd ed., Chapter 20 
##     it is part of the PyTree example
#########################################################################
...
#### totally uncool, but until I get the skinny on getting font metrics, use a width factor for longer node names
####  This will have to be adjusted based on the font
####  see TreeViewer.drawlevels
def node_width_factor(text):
    max_length  = 5.0  # assumes that 5 letters is the largest text label that shows well given current font
    wf = len(text)/max_length 
    return wf if wf > 1 else 1

Width, Height = 800, 600                    # start canvas size (reset per tree)
